...
\newcommand{\lecture}[3]{
   ...
   \framebox{
      \vbox{\vspace{2mm}
      \hbox to 6.28in {CSCI-4968: ML and Optimization \hfill Spring 2023 }
        \vspace{5mm}
        \hbox to 6.28in { {\Large \hfill #1 \hfill} }
        \vspace{2mm}
         \hbox to 6.28in { {\hfill \ \ \ \ \ #3 \hfill} }
        \vspace{2mm}
         \hbox to 6.28in { {\footnotesize{\url{https://github.com/Xinshi0726/MLOPT-U9-SDP-Relxation-For-TSP}} \hfill Rensselaer Polytechnic Institute}}
        % \vspace{2mm}
      }
   }
   \end{center}
...
With linear programming, we can optimize over  \( n \) variables. Now, suppose we have \( n^2 \) elements. Thus instead of finding an optimal \( \vec{x} \in \mathbb{R} \) we are trying to find an optimal \( \mathbf{X} \in \mathbb{R}^{n \times n} \) that is symmetric positive semidefinite (SPSD) such that
\begin{equation*}
  \begin{array}{ll@{}ll}
